...
## 🗄️ ৯. ডাটাবেজ ইনিশিয়ালাইজেশন ও রিলেশনাল মডেল (Database Initialization & Relational Model)

রিয়েল-ওয়ার্ল্ডে রিলেশনাল ডাটাবেজের (RDBMS) সঠিক ডিজাইন এবং কনস্ট্রেইন্টসের ব্যবহার সিস্টেমের ডেটা ইন্টিগ্রিটি বা ডেটার বিশুদ্ধতা রক্ষা করতে সাহায্য করে। নিচে `users` এবং `orders` টেবিলের প্রতিটি কলাম এবং কনস্ট্রেইন্টের ডিপ-ডাইভ ব্যাখ্যা দেওয়া হলো:

### 👤 ক) `users` টেবিল ডিজাইন ও মেকানিক্স
ইউজারদের ডেটা সংরক্ষণের জন্য তৈরি করা এই টেবিলটির কুয়েরি:
```sql
...
#### ১. `SERIAL PRIMARY KEY`
- **`SERIAL`**: এটি একটি অটো-ইনক্রিমেন্টিং ইন্টিজার টাইপ। ডাটাবেজ ব্যাকএন্ডে একটি সিকোয়েন্স অবজেক্ট (`users_id_seq`) তৈরি করে এবং কলামটির ডিফল্ট ভ্যালু হিসেবে `nextval()` সেট করে। এটি মাল্টিপল রিকোয়েস্ট একসাথে এলেও ইউনিক আইডি জেনারেট নিশ্চিত করে।
...
### 📦 খ) `orders` টেবিল ও One-to-Many রিলেশনশিপ
```sql
...
#### ⭐ ১. রেফারেন্সিয়াল অ্যাকশন (`ON DELETE CASCADE`)
...
#### ২. ডাটাবেজ লেভেল ভ্যালিডেশন (`CHECK Constraint`)
...
#### 💸 ৩. আর্থিক হিসাবের জন্য টাইপ চয়েস (`NUMERIC` vs `FLOAT`)
...
### ⏳ গ) অ্যাসিনক্রোনাস স্টার্টআপ সিকোয়েন্স (Asynchronous Startup Sequence)
টাইপস্ক্রিপ্ট/জাভাস্ক্রিপ্ট একটি নন-ব্লকিং এনভায়রনমেন্ট। 
- আপনি যখন ডাটাবেজে টেবিল তৈরির জন্য `initDB()` কল করবেন, এটি ডাটাবেজে কুয়েরি পাঠাতে কিছুটা সময় নেবে।
- সার্ভার লিসেন করার আগে অবশ্যই `await initDB()` করতে হবে। যদি আমরা `await` না করে সার্ভার চালু করি, তবে টেবিল তৈরি হওয়ার আগেই যদি কোনো ক্লায়েন্ট রিকোয়েস্ট পাঠিয়ে দেয়, ডাটাবেজ `Table does not exist` এরর দেবে।
- আমরা `index.ts` ফাইলে `try-catch` দিয়ে এই ইনিশিয়ালাইজেশন সিকিউর করেছি। কোনো কারণে ডাটাবেজ কানেকশন ফেইল হলে সার্ভার `process.exit(1)` দিয়ে বন্ধ হয়ে যাবে, যা ব্যাকএন্ডের আর্কিটেকচারাল স্ট্যাবিলিটি বজায় রাখে।
